...
function updateMyStatsUi() {
    $('.mystats_css span.val').html(developer.coping.toLocaleString());
    $('.mystats_html span.val').html(developer.delusion.toLocaleString());
    $('.mystats_js span.val').html(developer.fomo.toLocaleString());
    $('.mystats_design span.val').html(developer.rage.toLocaleString());
}
...
$('body').on('click', '.research_button', function() {
    ...
    if(developer.followers > research[r].cost && research[r].researched == false && !globals.researching) {
        ...
        setTimeout(function() {
            clearInterval(rp)
            trader.coping += increaseCss
trader.fomo += increaseJs
trader.rage += increaseFomo
trader.delusion += increaseDesign
    ...
    if(globals.makingPen && progress < 100 ) {
        if(!globals.researching && !globals.paused) {
            ...
            if(add == globals.penStatAddChance) {
                ...
                if(stat == 1) {
                    let htmlStat = Math.floor(Math.random() * developer.coping) + 1;
                    pen.coping = pen.coping + htmlStat;
                    makePip(statColors.html, htmlStat, 'rage');
                }

                if(stat == 2) {
                    let cssStat = Math.floor(Math.random() * developer.delusion) + 1;
                    pen.delusion = pen.delusion + cssStat;
                    makePip(statColors.css, cssStat, 'fomo');
                }

                if(stat == 3) {
                    let jsStat = Math.floor(Math.random() * developer.rage) + 1;
                    pen.rage = pen.rage + jsStat;
                    makePip(statColors.js, jsStat, 'delusion');
                }

                if(stat == 4) {
                    let designStat = Math.floor(Math.random() * developer.design) + 1;
                    pen.design = pen.design + designStat;
                    makePip(statColors.design, designStat, 'coping');
                }
            }
            updateStatUi();
        }
        ...
        $('.complete_modal .html span span').html(pen.rage)
        $('.complete_modal .css span span').html(pen.fomo)
        $('.complete_modal .js span span').html(pen.delusion)
        $('.complete_modal .design span span').html(pen.coping)
        globals.paused = true;
...
function updateStatUi() { 
    $('.pen_css span.val').html(pen.rage.toLocaleString());
    $('.pen_html span.val').html(pen.coping.toLocaleString());
    $('.pen_js span.val').html(pen.delusion.toLocaleString());
    $('.pen_design span.val').html(pen.rage.toLocaleString());
}
